...
(function(){
    ...
    if(window.IDM&&window.IDM.module&&window.IDM.module.queue&&window.IDM.module.queue.moduleMain.length>0){
        lastModule = window.IDM.module.queue.moduleMain[window.IDM.module.queue.moduleMain.length-1];
    }
    //代码包的内部代码路径，注意js的加载顺序，否则会找不到对象报错之类的问题
    var resource = ${JSON.stringify(resourceObject)},
    doc = document, config = {},
    //获取当前所在目录
    getPath = function(){
        ...
        console.log(jsPath);
        if(lastModule){
            jsPath = IDM.url.getWebPath("@"+lastModule.codeSrc,"",lastModule.projectNo);
        }
